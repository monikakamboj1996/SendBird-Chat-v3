export const channelsReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'fetch-channels': {
      ...
      const filterChannerls = mergedChannels.filter(channels => {
        return JSON.parse(channels.data).officeBranch == module + office;
      });
      // console.log('mergedChannels', filterChannerls);
      // return {
      //   ...state,
      //   channelMap,
      ...
      return {
        ...state,
        channelMap,
        channels: filterChannerls,
        empty: filterChannerls.length === 0 ? 'Start conversation.' : '',
      };
    }
    case 'join-channel':
    case 'update-channel': {
